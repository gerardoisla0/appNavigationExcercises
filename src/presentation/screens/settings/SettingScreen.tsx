...
export const SettingScreen = () => {
  ...
  useEffect(()=> {
    //LLamar a firebase RT
    const messagesRt = database().ref('messages');

    const onValueChange = messagesRt.on('child_added', snapshot => {
      const newMessage = snapshot.val();
      setMessages(prevMessages => [newMessage, ...prevMessages]);
    });

    return () => messagesRt.off('child_added', onValueChange);
  ...
  const sendMessage = async () => {
    ...
    const response = await fetch('http://192.168.18.22:3000/api/notification/message', {
      method: 'POST',
      headers: {
